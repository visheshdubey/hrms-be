...
function resolvePortalType(portalType, accountType) {
    if (portalType)
        return portalType;
...
function roleForPortal(portal) {
    return portal === 'recruiter' ? 'recruiter_admin' : 'org_admin';
}
function normalizeInviteRole(role, portal) {
...
const RECRUITER_ROLES = ['recruiter_admin', 'recruited_staff'];
const ORG_ROLES = ['org_admin', 'org_staff'];
function assertRoleMatchesPortal(role, portal) {
...
auth.get('/organization', async (c) => {
    ...
    try {
        ...
        let defaults = {};
        try {
            defaults = JSON.parse(org[0].defaults || '{}');
        }
        catch { /* ignore */ }
        return c.json({
            id: org[0].id,
...
auth.put('/organization', zValidator('json', orgUpdateSchema), async (c) => {
    ...
    try {
        ...
        if (body.defaults != null)
            patch.defaults = JSON.stringify(body.defaults);
        const [updated] = await db.update(organizations).set(patch)
...
auth.post('/invite', zValidator('json', inviteSchema), async (c) => {
    ...
    try {
        ...
        const portal = (inviterUser.portalType ?? 'recruiter');
...
auth.patch('/users/:id/role', zValidator('json', z.object({ role: z.string() })), async (c) => {
    ...
    try {
        ...
        const portal = targetUser[0].portalType;
        const resolvedRole = normalizeInviteRole(role, portal);
        const portalError = assertRoleMatchesPortal(resolvedRole, portal);
        if (portalError)
            return c.json({ error: portalError }, 400);
