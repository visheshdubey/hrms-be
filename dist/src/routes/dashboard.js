...
dashboardRouter.get('/', requireAuth, async (c) => {
    try {
        ...
        pipelineDataRaw.forEach((row) => { if (row.stage)
            stagesMap[row.stage] = row.count; });
        ...
        const rawBuckets = await db
            .select({
            bucket: sql `strftime(${fmt}, ${candidates.createdAt})`,
            count: sql `count(${candidates.id})`,
        })
            .from(candidates)
            .where(candWhere)
        ...
        const bucketMap = {};
        rawBuckets.forEach((r) => { if (r.bucket)
            bucketMap[r.bucket] = r.count; });
